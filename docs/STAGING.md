...
The Admin build variable is set only on `boccone-admin` in `staging`:

- `VITE_API_URL` equal to the final public API URL.
...
```bash
curl --fail --silent --show-error https://boccone-api-staging.up.railway.app/api/health
curl --fail --silent --show-error -I https://boccone-admin-staging.up.railway.app/
```
